...
import { Download } from "lucide-react"; // ← add this import at the top

const handleDownloadPDF = async (registrationId: string) => {
  const input = document.getElementById(`pdf-content-${registrationId}`);
  if (!input) return;

  // Create a canvas with higher scale for sharper image
  const canvas = await html2canvas(input, {
    scale: 3,
    useCORS: true, // allows logo rendering correctly
  ...
  // Compute image width and height keeping aspect ratio
  ...
  pdf.save(`student-portal-grades-${registrationId}.pdf`);
};
              ...
              <TableRow className="bg-white">
                <TableCell colSpan={5}>
                  <div className="h-fit w-full bg-white p-2">
                    {/* 🔘 PRINT BUTTON */}
                    {/* 🔘 PRINT BUTTON — only show if there are grades */}
                    {grades.length > 0 && (
                      <div className="flex justify-end mb-2">
                        <button
                          onClick={() =>
                            handleDownloadPDF(studentReg.registration_id)
                          }
                          className="flex items-center gap-1 bg-blue-500 hover:bg-blue-600 text-white text-sm px-3 py-1 rounded-md transition-all"
                        >
                    ...
                    <div
                      id={`pdf-content-${studentReg.registration_id}`}
                      style={{ paddingBottom: "40px" }} // ✅ prevent last line cutoff
                    >
