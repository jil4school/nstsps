...
export const ForgotPasswordProvider = ({ children }: { children: ReactNode }) => {
  ...
  const sendVerification = async () => {
    try {
      ...
      const sendRes = await axios.post(API_URL, { type: "sendCode", email, code });
      ...
      toast.error(err.response?.data?.error || "Error sending verification code");
    }
  };

  const verifyCode = async (code: string) => {
    try {
      const res = await axios.post(API_URL, { type: "verifyCode", email, code });
      if (res.data.success) {
  ...
  const changePassword = async (newPassword: string, confirmPassword: string) => {
    ...
    try {
      const res = await axios.post(API_URL, { type: "changePassword", email, new_password: newPassword });
      if (res.data.success) {
...
export const useForgotPassword = () => {
  ...
  if (!context) throw new Error("useForgotPassword must be used within ForgotPasswordProvider");
  return context;
};
